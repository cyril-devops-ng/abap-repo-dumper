...
parameters:
  _payment RADIOBUTTON GROUP G1,
  _receipt RADIOBUTTON GROUP G1.
...
*            clear containers to be used for transaction
    if ( NOT cash_j_line-COMP_CODE ca 'COMP' ).
    clear:
      HEADER, items[], return[].
...
*     ACCEPT_INITIAL_DATE      =
  ...
  header-ALLOC_NMBR = cash_j_line-ALLOC_NUMBER.
...
form prepareItems.
  ...
  ELSE.
    items-P_RECEIPTS = cash_j_line-H_PAYMENTS.
  endif.
